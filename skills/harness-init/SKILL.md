...
Agent(
  ...
  description="프로젝트 분석",
  prompt="<analyzer 에이전트 지침에 따라 분석. 프로젝트 루트: [절대경로]. mode: [lite/init]. tier: [Lite/Standard/Full]. 결과: _workspace/01_analyzer_report.md + (Standard/Full만) _workspace/index/*.json>",
  model="[sonnet/opus]"
)
```
